...
for (const [file, claimSource] of ARTIFACTS) {
  ...
  /* An artifact that yields no rows at all is a shape change, not a corpus
   * with nothing in it: every file listed above is a scorecard, and a scorecard
   * has rows by construction. Without this, renaming `admitted`/`rejections`/
   * `rows` in the harness leaves both branches above reading `[]`, this script
   * printing a smaller label set, and `measure-agreement.mjs` scoring it
   * without complaint -- a merge that silently stopped merging one of its four
   * sources, under a README that says these numbers can be re-derived. */
  if (rows.length === 0) {
...
/* And the merged set, which `--executed-only` can empty on its own. Emitting
 * zero labels exits 0 and hands `measure-agreement.mjs` a file to score. */
if (rows.length === 0) {
  console.error('no labels survived the merge; there is nothing to score');
  process.exit(2);
}
